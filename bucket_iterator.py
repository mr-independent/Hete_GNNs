...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        for item in batch_data:
            ...
            text_raw_bert_indices, aspect_bert_indices, bert_segments_ids = \
                item['text_raw_bert_indices'], item['aspect_bert_indices'], item['bert_segments_ids']
            text_bert_indices = item['text_bert_indices']

            # text_padding = [0] * (max_len - len(text_indices))
            # test
            text_padding = [0] * (max_len - len(text_indices) + 5)
            ...
            text_raw_bert_padding = [0] * (max_len - len(text_raw_bert_indices))
            aspect_bert_padding = [0] * (max_len - len(aspect_bert_indices))
            bert_segments_ids_padding = [0] * (max_len - len(bert_segments_ids))
            ...
            # batch_polarity.append(polarity)
            # batch_text_raw_bert_indices.append(text_raw_bert_indices )
            # batch_aspect_bert_indices.append(aspect_bert_indices )
