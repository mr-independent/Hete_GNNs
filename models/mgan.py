...
class MGAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ctx_out, (_, _) = self.ctx_lstm(ctx, ctx_len.cpu())
        ...
        asp_out, (_, _) = self.asp_lstm(asp, asp_len.cpu()) # batch_size x (asp)seq_len x 2*hidden_dim
        ...
        ctx_out = ctx_out.to(torch.float32)
        ...
        c_ctx2asp_alpha = F.softmax(asp_out.matmul(self.w_c2a.expand(batch_size, -1, -1)).matmul(ctx_pool.to(torch.float32)), dim=1)
