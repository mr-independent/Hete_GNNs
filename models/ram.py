...
class RAM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        memory, (_, _) = self.bi_lstm_context(memory, memory_len.cpu())
        ...
        for _ in range(self.opt.hops):
            g = self.att_linear(torch.cat([memory.to(torch.float32),
            ...
            i = torch.bmm(alpha.transpose(1, 2), memory.to(torch.float32)).squeeze(1)
